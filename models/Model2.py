...
class Up(nn.Module):
    ...
    def __init__(self, ch_in_lower: int, ch_in_skip: int, ch_out_doubleconv: int, bilinear: bool = True):
        super().__init__()
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(ch_in_lower, ch_in_lower // 2, kernel_size=2, stride=2)
            # After concat, it's (ch_in_lower // 2) + ch_in_skip
            self.conv = DoubleConv((ch_in_lower // 2) + ch_in_skip, ch_out_doubleconv)

    def forward(self, x1: torch.Tensor, x2: torch.Tensor) -> torch.Tensor: # x1 from lower (e.g. x5), x2 from skip (e.g. x4)
        x1 = self.up(x1)
        # input is CHW
...
class Model2(BaseModel):
    ...
    def __init__(self, n_channels: int = 3, n_classes: int = 1, bilinear: bool = True, device: Optional[str] = None):
        ...
        self.up1 = Up(1024, 512, 512, bilinear) # Input from down4 (1024), skip from down3 (512) -> Output 512
        self.up2 = Up(512, 256, 256, bilinear)  # Input from up1 (512), skip from down2 (256) -> Output 256
        self.up3 = Up(256, 128, 128, bilinear)  # Input from up2 (256), skip from down1 (128) -> Output 128
        self.up4 = Up(128, 64, 64, bilinear)   # Input from up3 (128), skip from inc (64) -> Output 64
