        ...
        try:
            ...
            if optimizer and 'optimizer_state_dict' in checkpoint:
                try:
                    ...
                    print(f"Loaded checkpoint including optimizer state from {model_path} (resuming from epoch {start_epoch})")
                except Exception as e:
                    print(f"Error loading optimizer state from {model_path}: {e}. Optimizer state not loaded.")
            else:
                print(f"Loaded model state checkpoint from {model_path} (resuming from epoch {start_epoch})")
                if optimizer and 'optimizer_state_dict' not in checkpoint:
                    print("Optimizer state not found in checkpoint. Optimizer not loaded.")
            ...
            print(f"Error loading checkpoint from {model_path}: {e}. Starting from scratch.")
            return 0
