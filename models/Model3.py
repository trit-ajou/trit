...
class Down_M3(nn.Module):
    ...
    def __init__(self, in_channels, out_channels):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleConv_M3(in_channels, out_channels)
        )
    def forward(self, x: torch.Tensor) -> torch.Tensor:
...
class Up_M3(nn.Module):
    ...
    def __init__(self, ch_in_lower: int, ch_in_skip: int, ch_out_doubleconv: int, bilinear: bool = True):
        ...
        if bilinear:
            ...
            self.conv = DoubleConv_M3((ch_in_lower // 2) + ch_in_skip, ch_out_doubleconv)
...
class Model3(BaseModel):
    ...
    def __init__(self, img_channels: int = 3, mask_channels: int = 1, output_channels: int = 3, 
                 bilinear: bool = True, device: Optional[str] = None):
        ...
        self.final_activation = torch.sigmoid # Assuming output pixel values are in [0, 1]

        self.to(self.device) # Ensure model is on the device specified in BaseModel

    def forward(self, image_with_text: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
