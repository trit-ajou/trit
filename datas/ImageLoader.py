...
class ImageLoader:
    ...
    def get_gen_images(self) -> List[TextedImage]:
        if not self.future:
            raise ValueError("[ImageLoader] 오류: 시작된 생성 작업이 없습니다.")
        # 작업이 이미 완료되었다면 바로 결과 반환
        if self.future.done():
            try:
                return self.future.result()
            except Exception as e:
                raise ValueError(f"[ImageLoader] 이미지 생성 중 오류 발생: {e}")
    ...
    def _task_gen_images(
        ...
        if not self.setting.use_noise:
            ...
            print(f"[ImageLoader] 생성 가능 이미지 {len(_paths)}/{num_images}")
            for _path in _paths:
                clear_pils.append(Image.open(_path).convert("RGB"))
        # 부족한 만큼 노이즈 이미지 생성(model1 input size 크기)
    ...
    def pil_to_texted_image(
        ...
        all_char_infos_for_craft: Optional[List[CharInfo]] = (
            [] if generate_craft_gt else None
        )
        ...
        for i_block in range(num_render_texts):
            ...
            current_text_block_bbox = BBox(
                abs_x_on_img,
                abs_y_on_img,
                abs_x_on_img + text_pil_w,
            ...
            timg_tensor = TextedImage._alpha_blend(
                timg_tensor, current_text_block_bbox, rgb_pil_tensor, alpha_pil_tensor
            )

            current_text_mask_tensor = (alpha_pil_tensor > 0).float()
            text_block_pixel_mask[
                :,
                current_text_block_bbox.y1 : current_text_block_bbox.y2,
            ...
            if generate_craft_gt and char_infos_relative_to_block_pil:
                for char_info_rel in char_infos_relative_to_block_pil:
                    abs_polygon = char_info_rel.polygon + np.array(
                        [abs_x_on_img, abs_y_on_img]
                    )
                ...
                max_rel_word_id = max(
                    ci.word_id for ci in char_infos_relative_to_block_pil
                )
    ...
    def _render_text_block_and_get_char_infos(
        ...
        if random.random() < self.policy.shadow_prob:
            ...
            shadow_params_val = (
                int(font_obj.size * s_off_x_r),
                int(font_obj.size * s_off_y_r),
                s_blur,
                s_color,
            )
        ...
        try:
            text_bbox_on_pil = _draw_dummy.textbbox(
                (0, 0),
                final_text_str,
                font=font_obj,
                stroke_width=stroke_w_val,
                spacing=line_spacing_pixels,
                align=text_align_opt,
            )
        except TypeError:  # 구버전 Pillow 호환
            text_bbox_on_pil = _draw_dummy.textbbox(
                (0, 0), final_text_str, font=font_obj
            )
            if stroke_w_val > 0:
                text_bbox_on_pil = (
                    text_bbox_on_pil[0] - stroke_w_val,
                    text_bbox_on_pil[1] - stroke_w_val,
                    text_bbox_on_pil[2] + stroke_w_val,
                    text_bbox_on_pil[3] + stroke_w_val,
                )

        text_render_w, text_render_h = (
            text_bbox_on_pil[2] - text_bbox_on_pil[0],
            text_bbox_on_pil[3] - text_bbox_on_pil[1],
        )
        if text_render_w <= 0 or text_render_h <= 0:
            return None, None

        padding_factor = 1.0  # 변형 시 잘림 방지를 위해 넉넉한 패딩
        pad_x = (
            int(text_render_w * padding_factor)
            + stroke_w_val
            + (
                abs(shadow_params_val[0]) + shadow_params_val[2]
                if shadow_params_val
                else 0
            )
        )
        pad_y = (
            int(text_render_h * padding_factor)
            + stroke_w_val
            + (
                abs(shadow_params_val[1]) + shadow_params_val[2]
                if shadow_params_val
                else 0
            )
        )

        layer_w, layer_h = int(max(1, text_render_w + pad_x * 2)), int(
            max(1, text_render_h + pad_y * 2)
        )
        draw_x_origin, draw_y_origin = (
            pad_x - text_bbox_on_pil[0],
            pad_y - text_bbox_on_pil[1],
        )

        # 1-3. 텍스트를 초기 레이어에 렌더링
        text_layer_img_untransformed = Image.new(
            "RGBA", (layer_w, layer_h), (0, 0, 0, 0)
        )
        draw_on_layer = ImageDraw.Draw(text_layer_img_untransformed)
        if shadow_params_val:
            s_off_x, s_off_y, s_blur_r, s_color = shadow_params_val
            shadow_layer = Image.new(
                "RGBA", text_layer_img_untransformed.size, (0, 0, 0, 0)
            )
            shadow_draw = ImageDraw.Draw(shadow_layer)
            try:
                shadow_draw.text(
                    (draw_x_origin + s_off_x, draw_y_origin + s_off_y),
                    final_text_str,
                    font=font_obj,
                    fill=s_color,
                    stroke_width=stroke_w_val,
            ...
            if s_blur_r > 0:
                shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(s_blur_r))
        ...
        for line_text_str in lines:
            if not line_text_str.strip():
                try:
                    empty_line_h = (
                        font_obj.getmask("A").size[1]
                        if hasattr(font_obj.getmask("A"), "size")
                        else font_obj.getsize("A")[1]
                    )
            ...
            try:
                line_bbox_for_align = _draw_dummy.textbbox(
                    (0, 0),
                    line_text_str,
                    font=font_obj,
                    stroke_width=stroke_w_val,
                ...
                line_actual_render_width = (
                    line_bbox_for_align[2] - line_bbox_for_align[0]
                )
                line_actual_render_height = (
                    line_bbox_for_align[3] - line_bbox_for_align[1]
                )
            except TypeError:
                line_actual_render_width = (
                    font_obj.getsize(line_text_str)[0]
            ...
            if text_align_opt == "center":
                current_line_x_start_in_padded += (
                    text_render_w - line_actual_render_width
                ) / 2.0
            ...
            for char_val in line_text_str:
                try:
                    char_font_bbox = font_obj.getbbox(
                        char_val, stroke_width=stroke_w_val
                    )
                    ...
                    char_advance = (
                        font_obj.getsize(char_val)[0]
                        if hasattr(font_obj, "getsize")
                        else (char_font_bbox[2] - char_font_bbox[0])
                    )

                char_x1 = (
                    current_line_x_start_in_padded
                    + current_char_x_offset_in_line
                    + char_font_bbox[0]
                )
            ...
            current_line_y_start_in_padded += (
                line_actual_render_height + line_spacing_pixels
            )
            current_relative_word_id_in_block += 1
        ...
        if debug_visualization_prefix and untransformed_char_polygons_with_info:
            ...
            for poly, _, _ in untransformed_char_polygons_with_info:
                draw_vis_untransformed.polygon(
                    [tuple(p) for p in poly.tolist()], outline=(0, 255, 0, 128), width=1
                )
                center_x, center_y = np.mean(poly[:, 0]), np.mean(poly[:, 1])
                r = 2
                draw_vis_untransformed.ellipse(
            ...
            img_vis_untransformed.save(
                os.path.join(
                    vis_save_dir,
                    f"{debug_visualization_prefix}_untransformed_chars.png",
                )
            )
        ...
        T_to_origin = np.array(
            [[1, 0, -center_x0], [0, 1, -center_y0], [0, 0, 1]], dtype=np.float64
        )
        ...
        T_to_new_center = np.array(
            [[1, 0, center_x1], [0, 1, center_y1], [0, 0, 1]], dtype=np.float64
        )
        ...
        # 3-2. 기울이기(Shear)
        if random.random() < self.policy.shear_apply_prob:
        ...
        # 3-3. 원근(Perspective)
        if random.random() < self.policy.perspective_transform_enabled_prob:
            H_forward_coeffs = self._get_perspective_coeffs_numpy(
                img_to_transform_pil.size[0],
                img_to_transform_pil.size[1],
                self.policy.perspective_transform_strength_ratio_range,
            )
            if H_forward_coeffs:
                M_perspective_forward = np.array(
                    [
                        [H_forward_coeffs[0], H_forward_coeffs[1], H_forward_coeffs[2]],
                        [H_forward_coeffs[3], H_forward_coeffs[4], H_forward_coeffs[5]],
                        [H_forward_coeffs[6], H_forward_coeffs[7], 1.0],
                    ],
                    dtype=np.float64,
                )
                try:
        ...
        if debug_visualization_prefix and untransformed_char_polygons_with_info:
            ...
            for poly_untransformed, _, _ in untransformed_char_polygons_with_info:
                poly_h = np.hstack(
                    [poly_untransformed, np.ones((poly_untransformed.shape[0], 1))]
                )
                transformed_poly_h = (T_coords_total @ poly_h.T).T
                w_coords = transformed_poly_h[:, 2]
                if np.any(np.abs(w_coords) < 1e-7):
                    continue
                transformed_poly = transformed_poly_h[:, :2] / w_coords[:, np.newaxis]

                draw_vis_transformed.polygon(
                    [tuple(p) for p in transformed_poly.tolist()],
                    outline=(0, 0, 255, 128),
                    width=1,
                )
                center_x, center_y = np.mean(transformed_poly[:, 0]), np.mean(
                    transformed_poly[:, 1]
                )
                r = 2
                draw_vis_transformed.ellipse(
                    (center_x - r, center_y - r, center_x + r, center_y + r),
                    fill=(255, 255, 0, 200),
                )

            vis_save_dir = self.setting.debug_dir
            img_vis_transformed.save(
                os.path.join(
                    vis_save_dir, f"{debug_visualization_prefix}_transformed_chars.png"
                )
            )
        ...
        if cropped_final_img_pil.width == 0 or cropped_final_img_pil.height == 0:
            return None, None
        ...
        if untransformed_char_polygons_with_info:
            ...
            for (
                ...
                char_s,
                rel_word_id,
            ) in untransformed_char_polygons_with_info:
                poly_h = np.hstack(
                    [poly_untransformed, np.ones((poly_untransformed.shape[0], 1))]
                )
                transformed_poly_h = (T_coords_total @ poly_h.T).T
                ...
                transformed_poly_cartesian = (
                    transformed_poly_h[:, :2] / w_coords[:, np.newaxis]
                )
                poly_relative_to_crop = transformed_poly_cartesian - np.array(
    ...
    def _create_craft_gt_maps(
        ...
        for char_info in all_char_infos_abs_coords:
            ...
            gaussian_values = np.exp(
                -(
                    (rotated_x**2 / (2 * variance_x_sq))
                    + (rotated_y**2 / (2 * variance_y_sq))
                )
            )
        ...
        for _, chars_in_word in words_data.items():
            if len(chars_in_word) < 2:
                continue

            # 문자를 x좌표 기준으로 정렬하여 처리 순서 결정
            sorted_chars = sorted(
                chars_in_word, key=lambda ci: (ci.polygon[0, 0], ci.polygon[0, 1])
            )
            ...
            for i in range(len(sorted_chars) - 1):
                ...
                aff_center_x, aff_center_y = (center1[0] + center2[0]) / 2.0, (
                    center1[1] + center2[1]
                ) / 2.0
                ...
                h1 = (
                    np.max(poly1[:, 1]) - np.min(poly1[:, 1])
                    if poly1.shape[0] > 0
                    else 0
                )
                h2 = (
                    np.max(poly2[:, 1]) - np.min(poly2[:, 1])
                    if poly2.shape[0] > 0
                    else 0
                )
                ...
                gaussian_values = np.exp(
                    -(
                        (rotated_x**2 / (2 * var_major_sq))
                        + (rotated_y**2 / (2 * var_minor_sq))
                    )
                )

                # 두 문자를 잇는 볼록 다각형(convex hull)에만 가우시안 적용
                mask = np.zeros_like(affinity_map_np, dtype=np.uint8)
                combined_poly = np.concatenate(
                    (poly1.astype(np.int32), poly2.astype(np.int32)), axis=0
                )
                if combined_poly.shape[0] >= 3:
                    try:
                        ...
                        cv2.error
                    ):  # 점이 너무 적거나 일직선상에 있어 hull을 만들 수 없는 경우
                        cv2.line(
                            mask,
                            tuple(center1.astype(np.int32)),
                            tuple(center2.astype(np.int32)),
                            1,
        ...
        affinity_map_tensor = torch.from_numpy(
            np.clip(affinity_map_np, 0, 1)
        ).unsqueeze(0)
    ...
    def _render_text_layer(
        self, text_content: str, font: ImageFont.FreeTypeFont
